...
up-gke-prod:
	...
	gcloud container node-pools create consumer-pool --cluster k8s-1m --num-nodes 0 --enable-autoscaling --min-nodes 0 --max-nodes 5
	gcloud container node-pools create client-pool --cluster k8s-1m --num-nodes 0 --enable-autoscaling --min-nodes 0 --max-nodes 5
